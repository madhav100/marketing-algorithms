...
function parseArgs(argv) {
  const args = { topology: 'hybrid', fps: 20, depth: 3, trace: '', liveLocalhost: false, dryRun: false };
  for (let i = 2; i < argv.length; i += 1) {
    ...
    else if (a === '--live-localhost') args.liveLocalhost = true;
...
function resolveTracePath(opts) {
  if (opts.trace) return opts.trace;
  if (opts.liveLocalhost) {
    return path.resolve(__dirname, '../../salesforce commerce cloud/server/runtime-flow/events.jsonl');
  }
  return '';
}
...
function main() {
  ...
  opts.trace = resolveTracePath(opts);
  ...
  if (opts.dryRun) {
    console.log(JSON.stringify({
      ...
      liveLocalhost: opts.liveLocalhost,
    }, null, 2));
    return;
  }
