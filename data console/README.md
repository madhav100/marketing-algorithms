...
- `POST /api/ingest` — triggers ingestion and returns updated summary + DMO analytics payload.
- `GET /api/summary` — returns current `lake-summary.json` (generated at ingest time, including model object counts).
- `GET /api/dmo-analytics` — returns chart-ready analytics from `data cloud/data/dmo-analytics.json`.

## Generated artifacts
- `data cloud/data/objects-lake.json`, `data cloud/data/model-objects.json`, `data cloud/data/dmo-analytics.json`, and `data cloud/data/lake-summary.json` are generated by ingestion and should not be manually edited.
